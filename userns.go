...
func remountReadOnlyRecursive(path string, debug bool) error {
	mountInfo, err := parseMountInfo(path, "/newroot/proc/self/mountinfo")
	if err != nil {
...
func mountBindOld(source string, target string, readOnly bool, debug bool) error {
	...
	if readOnly {
		if err := remountReadOnlyRecursive(target, debug); err != nil {
			return err
		}
	}
